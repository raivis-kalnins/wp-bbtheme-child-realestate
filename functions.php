...
function wpbb_realestate_home_search( $content ) {
	return $content . '<!-- wp:wpbb/bootstrap-div {"containerClass":"","utilityClasses":"wp-theme-property-search-section","className":"wpbb-v62-section"} --><div class="container"><!-- wp:wpbb/sector-finder {"context":"realestate","limit":6} /--></div><!-- /wp:wpbb/bootstrap-div -->';
}
...
function wpbb_realestate_visual_icon_config() {
    $config = array( 'base' => get_stylesheet_directory_uri(), 'icons' => array('home', 'building', 'ruler-measure', 'map-pin', 'camera', 'chart-line', 'briefcase', 'calendar') );
    echo '<script>window.wpbbChildVisuals=' . wp_json_encode( $config ) . ';</script>';
}
add_action( 'wp_footer', 'wpbb_realestate_visual_icon_config', 1 );


/* v3.8.10.30: realistic demo blog featured images. Runs only after the theme's explicit demo import. */
function wpbb_realestate_demo_blog_photo_attachment( $filename, $title ) {
    $slug = sanitize_title( pathinfo( $filename, PATHINFO_FILENAME ) );
    $existing = get_page_by_path( 'realestate-blog-' . $slug, OBJECT, 'attachment' );
    if ( $existing ) {
        if ( function_exists( 'wpbb_realestate_refresh_bundled_attachment_v381041' ) ) wpbb_realestate_refresh_bundled_attachment_v381041( (int) $existing->ID, 'assets/img/blog' );
        return (int) $existing->ID;
    }
    $source = get_stylesheet_directory() . '/assets/img/blog/' . basename( $filename );
    if ( ! is_readable( $source ) ) return 0;
    $uploads = wp_upload_dir();
    $dir = trailingslashit( $uploads['basedir'] ) . 'realestate-blog';
    wp_mkdir_p( $dir );
    ...
    $id = wp_insert_attachment( array(
        ...
        'post_title' => $title,
        'post_name' => 'realestate-blog-' . $slug,
        'post_status' => 'inherit',
    ), $target );
...
function wpbb_realestate_seed_demo_blog_photos( $page_id = 0, $profile = array() ) {
    ...
    foreach ( $posts as $index => $post ) {
        ...
        $attachment = wpbb_realestate_demo_blog_photo_attachment( $filename, get_the_title( $post ) );
        if ( $attachment ) set_post_thumbnail( $post->ID, $attachment );
    }
}
add_action( 'wp_theme_after_demo_import', 'wpbb_realestate_seed_demo_blog_photos', 70, 2 );
 ...
 */
function wpbb_realestate_refresh_bundled_attachment_v381041( $attachment_id, $asset_dir ) {
...
function wpbb_realestate_realistic_media_upgrade_v381041() {
    if ( ( function_exists( 'wp_doing_ajax' ) && wp_doing_ajax() ) || ( function_exists( 'wp_doing_cron' ) && wp_doing_cron() ) ) return;
    $done_key = 'wpbb_realestate_realistic_media_upgrade_v381041';
    if ( get_option( $done_key ) ) return;
    if ( ! current_user_can( 'manage_options' ) ) return;

    $pairs = array(array('realestate-blog','assets/img/blog'));
    foreach ( $pairs as $pair ) {
        ...
        foreach ( $ids as $attachment_id ) {
            wpbb_realestate_refresh_bundled_attachment_v381041( $attachment_id, $asset_dir );
        }
    }
    if ( function_exists( 'wpbb_realestate_seed_demo_blog_photos' ) ) wpbb_realestate_seed_demo_blog_photos( 0, array() );
    if ( function_exists( 'wpbb_realestate_seed_properties' ) ) wpbb_realestate_seed_properties();
    foreach ( glob( get_stylesheet_directory() . '/assets/img/properties/*.jpg' ) ?: array() as $source ) {
        $slug = sanitize_title( pathinfo( $source, PATHINFO_FILENAME ) );
        $existing = get_posts( array( 'post_type'=>'attachment', 'name'=>'demo-property-image-' . $slug, 'post_status'=>'inherit', 'posts_per_page'=>1, 'fields'=>'ids' ) );
...
add_action( 'admin_init', 'wpbb_realestate_realistic_media_upgrade_v381041', 120 );
